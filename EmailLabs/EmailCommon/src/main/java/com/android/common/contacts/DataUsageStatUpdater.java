...
public class DataUsageStatUpdater {
    ...
    public boolean updateWithAddress(Collection<String> addresses) {
        //EmailLog.d(TAG, "updateWithAddress: " + Arrays.toString(addresses.toArray()));

        if (addresses != null && !addresses.isEmpty()) {
			EmailLog.d(TAG, "updateWithAddress");
            ...
            if (cursor == null) {
                EmailLog.w(TAG, "Cursor for Email.CONTENT_URI became null.");
            } else {
    ...
    public boolean updateWithPhoneNumber(Collection<String> numbers) {
        //EmailLog.d(TAG, "updateWithPhoneNumber: " + Arrays.toString(numbers.toArray()));
        ...
        if (numbers != null && !numbers.isEmpty()) {
            ...
            if (cursor == null) {
                EmailLog.w(TAG, "Cursor for Phone.CONTENT_URI became null.");
            } else {
                final Set<Long> contactIds = new HashSet<Long>(cursor.getCount());
                final Set<Long> dataIds = new HashSet<Long>(cursor.getCount());
                try {
    ...
    private boolean update(Collection<Long> contactIds, Collection<Long> dataIds, String type) {
        ...
        if (Build.VERSION.SDK_INT >= 14) {
            if (dataIds.isEmpty()) {
                EmailLog.d(TAG, "Given list for data IDs is null. Ignoring.");
            } else {
                ...
                if (mResolver.update(uri, new ContentValues(), null, null) > 0) {
                    ...
                    EmailLog.d(TAG, "update toward data rows " + dataIds + " failed");
                }
            }
        } else {
            // Use older API.
            if (contactIds.isEmpty()) {
                EmailLog.d(TAG, "Given list for contact IDs is null. Ignoring.");
            } else {
                final StringBuilder whereBuilder = new StringBuilder();
                final ArrayList<String> whereArgs = new ArrayList<String>();
                ...
                EmailLog.d(TAG, "contactId where: " + whereBuilder.toString());
                EmailLog.d(TAG, "contactId selection: " + whereArgs);
                ...
                if (mResolver.update(ContactsContract.Contacts.CONTENT_URI, values,
                    ...
                    EmailLog.d(TAG, "update toward raw contacts " + contactIds + " failed");
                }
            }
        }
