...
/**
 * This class represent email address. RFC822 email address may have following
 * format. "name" <address> (comment) "name" <address> name <address> address
 * Name and comment part should be MIME/base64 encoded in header if necessary.
 */
public class Address implements Comparable<Address>{
    /**
     * Address part, in the form local_part@domain_part. No surrounding angle
     * brackets.
     */
    private String mAddress;

    /**
     * Name part. No surrounding double quote, and no MIME/base64 encoding. This
     * must be null if Address has no name part.
     */
    ...
    /**
     * Get name part as UTF-16 string. No surrounding double quote, and no
    ...
    /**
     * Set name part from UTF-16 string. Optional surrounding double quote will
     * be removed. It will be also unquoted and MIME/base64 decoded.
     * 
     * @param Personal name part of email address as UTF-16 string. Null is
     *            acceptable.
     */
    public void setPersonal(String personal) {
        if (personal != null) {
            ...
            /* [CIS] ds7.hwang 16.Aug.2010 */
            personal = DecoderUtil.decodeEncodedWords(personal).trim();
    ...
    /**
     * This method is used to check that all the addresses that the user entered
     * in a list (e.g. To:) are valid, so that none is dropped.
     */
    ...
    public static String getInvalidAddress(String addressList) {
            ...
            for (int i = 0, length = tokens.length; i < length; ++i) {
                ...
                if (!TextUtils.isEmpty(address) && !isValidAddress(address)) {
                    ...
                    if (name != null && name.length() > 0) {
                        retStr.append(token.toString());
                    } else {
                        retStr.append(address);
                    }
                }
            }
        }
    ...
    public static Address[] parse(String addressList) {
        ...
        for (int i = 0, length = tokens.length; i < length; ++i) {
            ...
            if (!TextUtils.isEmpty(address)) {
                // If STX character is present then tokenizer will fail
                // hence we have get the email address manually, otherwise
                // isValidAddress will return false even for valid address
                if (address.contains("\u0002")) {
    ...
    public static Address[] parseSMS(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return EMPTY_ADDRESS_ARRAY;
        }
        Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(addressList);
    ...
    public static String getName(String addressList) {
        String name = null;
        if (addressList == null || addressList.length() == 0) {
            return null;
        }
    ...
    /**
     * Checks whether a string email address is valid. E.g. name@domain.com is
     * valid.
     */
    /* package */public static boolean isValidAddress(String address) {
        ...
        int spaceAt = address.lastIndexOf(' ');

        if (spaceAt > 0 && spaceAt < (len - 1)) {
            return false;
        }

        return firstAt > 0 && firstAt == lastAt && lastAt + 1 < firstDot && firstDot <= lastDot
                && lastDot < len - 1;
    }
    ...
    public boolean equals(Object o) {
        if (o instanceof Address) {
            ...
            String curAddress = getAddress();
            String compAddress = ((Address) o).getAddress();

            if (isValidAddress(curAddress) && isValidAddress(compAddress)) {
                String curLocalPart = curAddress.substring(0, curAddress.indexOf('@'));
                String compLocalPart = compAddress.substring(0, compAddress.indexOf('@'));

                // local part (sensitive)
                if (curLocalPart.equals(compLocalPart)) {
                    String curDomainPart = curAddress.substring(curAddress.indexOf('@') + 1, curAddress.length());
                    String compDomainPart = compAddress.substring(compAddress.indexOf('@') + 1, compAddress.length());

                    // domain part (insensitive)
                    if (curDomainPart.equalsIgnoreCase(compDomainPart)) {
                        return true;
                    }
                }
            }
    ...
    public String toString() {
        if (mPersonal != null && !mPersonal.equals(mAddress)) {
            ...
            if (mPersonal.matches(".*[\\(\\)<>@,;:\\\\\".\\[\\]].*")) {
                return Utility.quoteString(mPersonal) + address;
            } else {
                return mPersonal + address;
            }
        } else {
            return mAddress;
        }
     ...
     * 
     * @return RFC822/MIME compatible address string. It may be surrounded by
     *         double quote or quoted and MIME/base64 encoded if necessary.
     */
     ...
     * 
     * @param addresses Address array
     * @return RFC822/MIME compatible comma-delimited address string. it may be
     *         surrounded by double quoted or quoted and MIME/base64 encoded if
     *         necessary.
     */
    ...
    /**
     * Creates a comma-delimited list of addresses in the "friendly" format (see
     * toFriendly() for details on the per-address conversion).
     * 
     * @param addresses Array of Address[] values
     * @return A comma-delimited string listing all of the addresses supplied.
    ...
    // Add minsumr.kim_1014 - remove "" in messagelist
    public static String toFriendlyUseInBubbleButton(Address[] addresses) {
        if (addresses == null || addresses.length == 0) {
            return null;
        }
        if (addresses.length == 1) {
            return addresses[0].toFriendly2();
        }
        StringBuffer sb = new StringBuffer(addresses[0].toFriendly2());
        for (int i = 1; i < addresses.length; i++) {
            sb.append(',');
            sb.append(addresses[i].toFriendly2());
        }
        return sb.toString();
    }

    public static String toFriendlyAddress(Address[] addresses) {
        if (addresses == null || addresses.length == 0) {
            return null;
        }
        if (addresses.length == 1) {
            return addresses[0].toFriendlyAddress();
        }
        StringBuffer sb = new StringBuffer(addresses[0].toFriendlyAddress());
        for (int i = 1; i < addresses.length; i++) {
            sb.append(',');
            sb.append(addresses[i].toFriendlyAddress());
        }
        return sb.toString();
    }

    public static int toFriendly1(Address[] addresses) {
        return addresses.length;
    }

    /**
     * Returns exactly the same result as
     * Address.toString(Address.unpack(packedList)).
     */
    ...
    /**
     * Returns null if the packedList has 0 addresses, otherwise returns the
     * first address. The same as Address.unpack(packedList)[0] for non-empty
     * list. This is an utility method that offers some performance optimization
     * opportunities.
     */
    ...
    /**
     * Convert a packed list of addresses to a form suitable for use in an
     * RFC822 header. This implementation is brute-force, and could be replaced
    ...
    public static Address[] unpack(String addressList) {
         ...
         * LIST_DELIMITER_PERSONAL is used, not for every email address; i.e.
         * not for every iteration of the while(). This reduces the theoretical
         * complexity from quadratic to linear, and provides some speed-up in
         * practice by removing redundant scans of the string.
         */
    ...
    public static Address[] unpack(String addressList, int capa) {
        if (addressList == null || addressList.length() == 0) {
            return EMPTY_ADDRESS_ARRAY;
        }
        ArrayList<Address> addresses = new ArrayList<Address>();
     ...
     * Packed lists can be unpacked with unpack(). The format is a series of
     * packed addresses separated by LIST_DELIMITER_EMAIL. Each address is
     * packed as a pair of address and personal separated by
     * LIST_DELIMITER_PERSONAL, where the personal and delimiter are optional.
    ...
    /**
     * Produces the same result as pack(array), but only packs one (this)
     * address.
     */
    public String pack() {
    ...
    /**
     * Legacy unpack() used for reading the old data (migration), as found in
     * LocalStore (Donut; db version up to 24).
     * 
     * @See unpack()
     */
    public static Address[] legacyUnpack(String addressList) {
        ...
        while (pairStartIndex < length) {
            ...
            if (addressEndIndex == -1 || addressEndIndex > pairEndIndex) {
                address = Utility
                        .fastUrlDecode(addressList.substring(pairStartIndex, pairEndIndex));
            } else {
                address = Utility.fastUrlDecode(addressList.substring(pairStartIndex,
                        addressEndIndex));
                personal = Utility.fastUrlDecode(addressList.substring(addressEndIndex + 1,
                        pairEndIndex));
            }
    ...
    /**
     * Legacy pack() used for writing to old data (migration), as found in
     * LocalStore (Donut; db version up to 24).
     * 
     * @See unpack()
     */
    ...
    //Added to make Address objects to be put in a Set.
    @Override
    public int compareTo(Address another) {
        return this.mAddress.compareToIgnoreCase(another.mAddress);
    }
}
