...
/**
 * Simple class used for debugging only that affords us a view of the raw IMAP
 * or POP3 stream, in addition to the tokenized version. Use of this class
 * *MUST* be restricted to logging-enabled situations only.
 */
public class LoggingInputStream extends FilterInputStream {
    ...
    public LoggingInputStream(InputStream in, String tag, boolean dumpEmptyLines) {
        ...
        initBuffer();
        EmailLog.d(Logging.LOG_TAG, mTag + "dump start");
    }
    ...
    private void logRaw(int oneByte) {
        if (oneByte == '\r') {
            ...
            // email protocols are supposed to be all 7bits, but there are wrong
            // implementations
    ...
    private void flushLog() {
        if (mDumpEmptyLines || (mSb.length() > mTag.length())) {
        	EmailLog.d(Logging.LOG_TAG, mSb.toString());
            initBuffer();
        }
    }
