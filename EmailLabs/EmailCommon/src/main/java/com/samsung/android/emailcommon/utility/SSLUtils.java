...
public class SSLUtils {
    ...
    /**
     * Returns a {@link SSLSocketFactory}. Optionally bypass all SSL certificate
     * checks.
     * 
    ...
    public synchronized static final SSLSocketFactory getSSLSocketFactory(boolean insecure) {
        if (insecure) {
            if (sInsecureFactory == null) {
                sInsecureFactory = SSLCertificateSocketFactory.getInsecure(SSL_HANDSHAKE_TIMEOUT, null);
            }
            return sInsecureFactory;
        } else {
            if (sSecureFactory == null) {
                sSecureFactory = SSLCertificateSocketFactory.getDefault(SSL_HANDSHAKE_TIMEOUT, null);
            }
