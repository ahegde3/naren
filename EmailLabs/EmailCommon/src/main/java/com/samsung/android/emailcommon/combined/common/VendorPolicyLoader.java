 ...
 * <p>
 * Email vendor policy is a system apk named "com.samsung.android.email.ui.helper". When
 * exists, it must contain a class called "com.samsung.android.email.ui.policy.EmailPolicy"
 * with a static public method <code>Bundle getPolicy(String, Bundle)</code>,
 * which serves vendor specific configurations.
 * <p>
 * A vendor policy apk is optional. The email application will operate properly
 * when none is found.
 */
public class VendorPolicyLoader {
	private static final String LOG_TAG = "VendorPolicyLoader";
    private static final String POLICY_PACKAGE = "com.samsung.android.email.ui.policy";
    ...
    public static VendorPolicyLoader getInstance(Context context) {
        if (sInstance == null) {
            // It's okay to instantiate VendorPolicyLoader multiple times. No
            // need to synchronize.
    ...
    /**
     * For testing only. Replaces the instance with a new instance that loads a
     * specified class.
     */
    public static void injectPolicyForTest(Context context, String apkPackageName, Class<?> clazz) {
        String name = clazz.getName();
        Log.d(LOG_TAG,
                String.format("Using policy: package=%s name=%s", apkPackageName, name));
        sInstance = new VendorPolicyLoader(context, apkPackageName, name, true);
    }

    /**
     * For testing only. Clear the instance so that the next
     * {@link #getInstance} call will return a regular, non-injected instance.
     */
    ...
    /**
     * Constructor for testing, where we need to use an alternate package/class
     * name, and skip the system apk check.
     */
    /* package */VendorPolicyLoader(Context context, String apkPackageName, String className,
        ...
        try {
            ...
            // Package not found -- it's okay - there's no policy .apk found,
            // which is OK
        } catch (ClassNotFoundException e) {
            // Class not found -- probably not OK, but let's not crash here
            Log.w(LOG_TAG, "VendorPolicyLoader: " + e);
    ...
    /**
     * Calls the getPolicy method in the policy apk, if one exists. This method
     * never returns null; It returns an empty {@link Bundle} when there is no
     * policy apk (or even if the inner getPolicy returns null).
     */
    // Not private for testing
    ...
    /**
     * Returns true if alternate exchange descriptive text is required. Vendor
     * function: Select: USE_ALTERNATE_EXCHANGE_STRINGS Params: none Result:
     * USE_ALTERNATE_EXCHANGE_STRINGS (boolean)
     */
    public boolean useAlternateExchangeStrings() {
        return getPolicy(USE_ALTERNATE_EXCHANGE_STRINGS, null).getBoolean(
                USE_ALTERNATE_EXCHANGE_STRINGS, false);
    }

    /**
     * Returns additional key/value pairs for the IMAP ID string. Vendor
     * function: Select: GET_IMAP_ID Params: GET_IMAP_ID_USER (String)
     * GET_IMAP_ID_HOST (String) GET_IMAP_ID_CAPABILITIES (String) Result:
     * GET_IMAP_ID (String)
     * 
     * @param userName the server that is being contacted (e.g.
     *            "imap.server.com")
     ...
     * @return zero or more key/value pairs, quoted and delimited by spaces. If
     *         there is nothing to add, return null.
     */
    ...
    /**
     * Returns provider setup information for a given email address Vendor
     * function: Select: FIND_PROVIDER Param: FIND_PROVIDER (String) Result:
     * FIND_PROVIDER_IN_URI FIND_PROVIDER_IN_USER FIND_PROVIDER_OUT_URI
     * FIND_PROVIDER_OUT_USER FIND_PROVIDER_NOTE (optional - null is OK) Note,
     * if we get this far, we expect "correct" results from the policy method.
     * But throwing checked exceptions requires a bunch of upstream changes, so
     * we're going to catch them here and add logging. Other exceptions may
     * escape here (such as null pointers) to fail fast.
     * 
    ...
    public Provider findProviderForDomain(String domain) {
        ...
        if (out != null && !out.isEmpty()) {
            ...
            p.setIncomingUriTemplate(out.getString(FIND_PROVIDER_IN_URI));
            p.incomingUsernameTemplate = out.getString(FIND_PROVIDER_IN_USER);
            // p.outgoingUriTemplate = out.getString(FIND_PROVIDER_OUT_URI);
            p.setOutgoingUriTemplate(out.getString(FIND_PROVIDER_OUT_URI));
    ...
    public static class Provider implements Serializable {
        ...
        public void setIncomingUriTemplate(String uri) {
        	
        	if (Utility.isMainlandChinaModel()
        			&& uri.contains("pop")) {
        		incomingUriTemplate_pop = uri;
        	} else {
        		incomingUriTemplate = uri;
        	}
            
        }

        public void setOutgoingUriTemplate(String uri) {
            outgoingUriTemplate = uri;
        }
    }
