...
//import com.samsung.android.emailcommon.EasRefs;
...
public class RecurrenceProcessor {
    ...
    // repeat event do not display .. When goto 1902.01.01~30 at timeline
    ...
    private static final int[] DAYS_IN_YEAR_PRECEDING_MONTH = {
            0, 31, 59, 90, 120, 151, 180, 212, 243, 273, 304, 334
    };

    /**
     * Returns the time (millis since epoch) of the last occurrence, or -1 if
     * the event repeats forever. If there are no occurrences (because the
     * exrule or exdates cancel all the occurrences) and the event does not
     * repeat forever, then 0 is returned. This computes a conservative estimate
     * of the last occurrence. That is, the time of the actual last occurrence
     * might be earlier than the time returned by this method.
     * 
     ...
     * 
     * @param dtstart the time of the first occurrence
     * @param maxtime the max possible time of the last occurrence. null means
     *            no limit
     * @param recur the recurrence
     * @return an estimate of the time (in UTC milliseconds) of the last
     *         occurrence, which may be greater than the actual last occurrence
    ...
    public long getLastOccurence(Time dtstart, Time maxtime, RecurrenceSet recur)
            throws DateException {
        long lastTime = -1;
        ...
        if (hasCount || recur.rdates != null || maxtime != null) {
            ...
            long[] dates = expand(dtstart, recur, dtstart.toMillis(false /*
                                                                          * use
                                                                          * isDst
                                                                          */) /*
                                                                               * range
                                                                               * start
                                                                               */,
                    (maxtime != null) ? maxtime.toMillis(false /* use isDst */) : -1 /*
                                                                                      * range
                                                                                      * end
                                                                                      */);
    ...
    /**
     * a -- list of values N -- number of values to use in a v -- value to check
     * for
     */
    ...
    /**
     * a -- list of values N -- number of values to use in a v -- value to check
     * for max -- if a value in a is negative, add that negative value to max
     * and compare that instead; this is how we deal with negative numbers being
     * offsets from the end value
     */
    ...
    /**
     * Filter out the ones for events whose BYxxx rule is for a period greater
     * than or equal to the period of the FREQ. Returns 0 if the event should
     * not be filtered out Returns something else (a rule number which is useful
     * for debugging) if the event should not be returned
     */
    private static int filter(EventRecurrence r, Time iterator) {
        ...
        if (EventRecurrence.MONTHLY >= freq) {
            // BYMONTH
            if (r.bymonthCount > 0) {
                found = listContains(r.bymonth, r.bymonthCount, iterator.month + 1);
                if (!found) {
                    return 1;
                }
            }
        ...
        if (EventRecurrence.WEEKLY >= freq) {
            ...
            if (r.byweeknoCount > 0) {
                found = listContains(r.byweekno, r.byweeknoCount, iterator.getWeekNumber(),
        ...
        if (EventRecurrence.DAILY >= freq) {
            ...
            if (r.byyeardayCount > 0) {
                found = listContains(r.byyearday, r.byyeardayCount, iterator.yearDay,
                        iterator.getActualMaximum(Time.YEAR_DAY));
                if (!found) {
                    return 3;
                }
            }
            ...
            if (r.bymonthdayCount > 0) {
                found = listContains(r.bymonthday, r.bymonthdayCount, iterator.monthDay,
        ...
        if (EventRecurrence.HOURLY >= freq) {
            // BYHOUR
            found = listContains(r.byhour, r.byhourCount, iterator.hour,
        ...
        if (EventRecurrence.MINUTELY >= freq) {
            // BYMINUTE
            found = listContains(r.byminute, r.byminuteCount, iterator.minute,
        ...
        if (EventRecurrence.SECONDLY >= freq) {
            // BYSECOND
            found = listContains(r.bysecond, r.bysecondCount, iterator.second,
        ...
        if (r.bysetposCount > 0) {
            bysetpos:
            // BYSETPOS - we only handle rules like
            ...
            // BYSETPOS was defined but we don't know how to handle it. Do no
    ...
    /**
     * Filters out instances that don't match the BYSETPOS clause of a monthly
     * recurrence rule. This is an awkward and inefficient way to go about it.
     * 
    ...
    private static boolean filterMonthlySetPos(EventRecurrence r, Time instance) {
         ...
         * "instance" has a day number and a DotW, so we compute the DotW of the
         * 1st from that. Note DotW is 0-6, where 0=SUNDAY. The basic
         * calculation is to take the instance's "day of the week" number,
         * subtract (day of the month - 1) mod 7, and then make sure it's
         * positive. We can simplify that with some algebra.
         */
        ...
        /*
         * Generate a set according to the BYDAY rules. For each day of the
         * month, determine if its day of the week is included. If so, append it
         * to the day set.
         */
        ...
        for (int md = 1; md <= maxDay; md++) {
            // For each month day, see if it's part of the set. (This makes some
            // assumptions
        ...
        /*
         * Now walk through the BYSETPOS list and see if the instance is equal
         * to any of the specified daySet entries.
         */
        for (int i = r.bysetposCount - 1; i >= 0; i--) {
            int index = r.bysetpos != null ? r.bysetpos[i] : 0;
            if (index > 0) {
    ...
    /**
     * Return whether we should make this list from the BYxxx list or from the
     * component of the iterator.
     */
    ...
    public static class DaySet {
        ...
        int getweek_day(Time iterator) {
            int day = 1;
            if (mR.bysetpos[0] == 1) { // first week day
                Time time = new Time(iterator);
                time.monthDay = 1;
                ...
                if (time.weekDay != Time.SUNDAY && time.weekDay != Time.SATURDAY)
                    ...
                    if (time.weekDay == Time.SUNDAY)
                        day = time.monthDay + 1;
                    else if (time.weekDay == Time.SATURDAY)
                        day = time.monthDay + 2;
                }
            } else if (mR.bysetpos[0] == 2) { // second weekday
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                if (time.weekDay == Time.MONDAY || time.weekDay == Time.TUESDAY
                        || time.weekDay == Time.WEDNESDAY || time.weekDay == Time.THURSDAY)
                    day = time.monthDay + 1;
                else {
                    if (time.weekDay == Time.FRIDAY)
                        ...
                        day = time.monthDay + 3;
                    else if (time.weekDay == Time.SUNDAY)
                        day = time.monthDay + 2;
                }
            } else if (mR.bysetpos[0] == 3) { // third week day
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                if (time.weekDay == Time.MONDAY || time.weekDay == Time.TUESDAY
                    ...
                    if (time.weekDay == Time.THURSDAY)
                        day = time.monthDay + 4;
                    else if (time.weekDay == Time.FRIDAY)
                        day = time.monthDay + 4;
                    else if (time.weekDay == Time.SATURDAY)
                        day = time.monthDay + 4;
                    else if (time.weekDay == Time.SUNDAY)
            ...
            } else if (mR.bysetpos[0] == 4) { // fourth week day
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                if (time.weekDay == Time.MONDAY || time.weekDay == Time.TUESDAY)
                    ...
                    if (time.weekDay == Time.WEDNESDAY)
                        day = time.monthDay + 5;
                    else if (time.weekDay == Time.THURSDAY)
                        day = time.monthDay + 5;
                    else if (time.weekDay == Time.FRIDAY)
                        day = time.monthDay + 5;
                    else if (time.weekDay == Time.SATURDAY)
            ...
            } else if (mR.bysetpos[0] == -1) { // last weekend
                Time time = new Time(iterator);
                int lastday;
                lastday = iterator.getActualMaximum(Time.MONTH_DAY);
                time.monthDay = lastday;
                unsafeNormalize(time);
                if (time.weekDay != Time.SUNDAY && time.weekDay != Time.SATURDAY)
                    day = lastday;
                else {
        ...
        // This method returns first weekend
        ...
        // This method returns second weekend
        int getsecondweekend(Time time) {
            int day = 1;
            day = getfirstweekend(time);
            Time time1 = new Time(time);
            time1.monthDay = day;
            unsafeNormalize(time1);
            if (time1.weekDay == Time.SATURDAY)
                day = day + 1;
            else if (time1.weekDay == Time.SUNDAY)
                day = day + 6;
            return day;
        }

        // This method returns third weekend
        int getthirdweekend(Time time) {
            int day = 1;
            day = getsecondweekend(time);
            Time time1 = new Time(time);
            time1.monthDay = day;
            unsafeNormalize(time1);
            if (time1.weekDay == Time.SATURDAY)
        ...
        // This method returns fourth weekend
        int getfourthweekend(Time time) {
            int day = 1;
            day = getthirdweekend(time);
            Time time1 = new Time(time);
            time1.monthDay = day;
            unsafeNormalize(time1);
            if (time1.weekDay == Time.SATURDAY)
        ...
        // This method returns Weekend means either staurday or sunday
        int getweekend_day(Time iterator) {
            int day = 1;
            if (mR.bysetpos[0] == 1) { // first weekend
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                day = getfirstweekend(time);
            } else if (mR.bysetpos[0] == 2) { // second weekend
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                day = getsecondweekend(time);
            } else if (mR.bysetpos[0] == 3) { // third weekend
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                day = getthirdweekend(time);
            } else if (mR.bysetpos[0] == 4) { // fourth weekend
                Time time = new Time(iterator);
                time.monthDay = 1;
                unsafeNormalize(time);
                day = getfourthweekend(time);
            } else if (mR.bysetpos[0] == -1) { // last weekend
                Time time = new Time(iterator);
                int lastday;
                lastday = iterator.getActualMaximum(Time.MONTH_DAY);
                ...
                if (time.weekDay == Time.SUNDAY || time.weekDay == Time.SATURDAY)
                    ...
                    day = lastday - time.weekDay;
                }
            }
            return day;
        }

        // This method returns last in month
        int getlastday(Time iterator) {
            int day = iterator.getActualMaximum(Time.MONTH_DAY);
            return day;
        }
        ...
        boolean get(Time iterator, int day) {
            ...
            if (EmailLog.DEBUG) {
                if (iterator.allDay) {
                    Log.i(TAG, "get called with allDay iterator with date" + iterator.month + "/"
                            + iterator.monthDay + "/" + iterator.year + " day=" + day);
                } else {
                    Log.i(TAG, "get called with iterator=" + iterator + " " + iterator.month + "/"
                            + iterator.monthDay + "/" + iterator.year + " day=" + day);
                }
            }
            if (day < 1 || day > 28) {
                // if might be past the end of the month, we need to normalize
                // it
                t = mTime;
                ...
                day = t.monthDay;
                if (EmailLog.DEBUG) {
                    Log.i(TAG, "normalized t=" + t + " " + t.month + "/" + t.monthDay + "/"
                            + t.year);
                }
            }
            if (realYear != mYear || realMonth != mMonth) {
                if (t == null) {
                    ...
                    unsafeNormalize(t);
                    if (EmailLog.DEBUG) {
                        Log.i(TAG, "set t=" + t + " " + t.month + "/" + t.monthDay + "/" + t.year
        ...
        /**
         * Fill in a bit set containing the days of the month on which this will
         * occur. Only call this if the r.freq > DAILY. Otherwise, we should be
        ...
        private static int generateDaysList(Time generated, EventRecurrence r) {
            ...
            if (count > 0) {
                ...
                // This isn't ideal, but we'll generate a month's worth of
                // events
                // and the code that calls this will only use the ones that
                // matter
                ...
                for (i = 0; i < count; i++) {
                    ...
                    if (v == 0) {
                        ...
                        for (; j <= lastDayThisMonth; j += 7) {
                            if (EmailLog.DEBUG)
                                Log.i(TAG, "setting " + j + " for rule " + v + "/"
                                        + EventRecurrence.day2TimeDay(byday[i]));
                            days |= 1 << j;
                        }
                    } else if (v > 0) {
                        ...
                        // v is negative
                        // should do +1 b/c the last one should add 0, but we
                        // also
     ...
     * milliseconds. The return value is never null. If there are no dates then
     ...
     */
    public long[] expand(Time dtstart, RecurrenceSet recur, long rangeStartMillis,
        ...
        // + repeat event do not display .. When goto 1902.01.01~30 at timeline
        if (rangeStartMillis < MINIMUM_START_RANGE_VALUE) {
            rangeStartMillis = MINIMUM_START_RANGE_VALUE;
        }
        // -
        ...
        if (recur.rrules != null) {
            for (EventRecurrence rrule : recur.rrules) {
                expand(dtstart, rrule, rangeStartDateValue, rangeEndDateValue, true /* add */,
                        dtSet);
            }
        }
        ...
        if (recur.exrules != null) {
            for (EventRecurrence exrule : recur.exrules) {
                expand(dtstart, exrule, rangeStartDateValue, rangeEndDateValue, false /* remove */,
                        dtSet);
            }
        }
        ...
        for (Long val : dtSet) {
            ...
            // change @siso kanchan P140626-02492 fix start
            try {
                dates[i] = mIterator.toMillis(true);
                i++;
            } catch (IllegalArgumentException e) {
                Log.i(TAG, "Setting hour minute and second to 0");
                mIterator.hour = 0;
                mIterator.minute = 0;
                mIterator.second = 0;
                dates[i] = mIterator.toMillis(true);
            ...
            // change @siso kanchan P140626-02492 fix end
        }
        return dates;
    }
     ...
     * timezone of the event. Return a list of iCalendar DATETIME strings
     * containing the start date/times of the occurrences; the output times are
     * defined in the local timezone of the event. If you want all of the
     * events, pass Long.MAX_VALUE for rangeEndDateValue. If you pass
     * Long.MAX_VALUE for rangeEnd, and the event doesn't have a COUNT or UNTIL
     * field, you'll get a DateException.
     * 
     * @param dtstart the dtstart date as defined in RFC2445. This {@link Time}
     *            should be in the timezone of the event.
     ...
     * @param rangeEndDateValue the last date-time you care about, not inclusive
     *            (so if you care about everything up through and including Dec
     *            22 1995, set last to Dec 23, 1995 00:00:00
     ...
     */
    public void expand(Time dtstart, EventRecurrence r, long rangeStartDateValue,
            long rangeEndDateValue, boolean add, TreeSet<Long> out) throws DateException {
        ...
        // For example, if dtstart is Mar 1, 2010 and the range is Jan 1 - Apr
        // 1,
        // then add it here and increment count. If the range is earlier or
        // later,
        ...
        //
        // NOTE: if DTSTART is not synchronized with the recurrence rule, the
        // first instance
        // we return will not fit the RRULE pattern.
        // this changes are to handle outlook events
        if (EmailLog.DEBUG)
            Log.i(TAG, "r.freq:" + r.freq);

        if (r.freq == EventRecurrence.MONTHLY) { // monthly outlook events
            if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 1) {
                // set the bysetpos vaue to bydaynum
                // bysetpos contains first(1), second(2), third(3), fourth(4),
                // last(-1) one of these value
                // ex: first monday , last saturday, second tuesday etc
                r.bydayNum[0] = r.bysetpos[0];
            } else if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 7) {
                // ex: first day or second day, day means any day like sunday to
                // friday
                if (r.bysetpos[0] == 1 || r.bysetpos[0] == 2 || r.bysetpos[0] == 3
                        || r.bysetpos[0] == 4) {
                    // this case is handled for first, second, third and fourth
                    // "bysetpos"
                    int[] bymonthday = new int[1];
                    bymonthday[0] = r.bysetpos[0];
                    r.bymonthday = bymonthday;
                    r.bymonthdayCount = 1;
                } else if (r.bysetpos[0] == -1) {
                    // this case is to handle last "bysetpos"
                    Monthly_Lastday = true;
                }
            } else if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 2) {
                // to handle, first, second, third, and last weekend
                // weekend means either saturday or sunday which come first,
                // that day will return.
                Monthly_weekend = true;
            } else if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 5) {
                // To handle first, second, third, fourth, last weekday means
                // between monday to friday
                Monthly_week_day = true;
            }
        } else if (r.freq == EventRecurrence.YEARLY) { // yearly
            if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 1) {
                // EX: first monday of jan, last saturday of jan
                r.bydayNum[0] = r.bysetpos[0];
                yearly_anyday = true;
            } else if (r.bysetpos != null && r.bysetposCount == 1 && r.bydayCount == 5) {
                // first weekday
                ...
                if (r.bysetpos[0] == 1 || r.bysetpos[0] == 2 || r.bysetpos[0] == 3
                        || r.bysetpos[0] == 4) {
                    // to handle first, second, third and fourth anyone day
                    int[] bymonthday = new int[1];
                    bymonthday[0] = r.bysetpos[0];
                    r.bymonthday = bymonthday;
                    r.bymonthdayCount = 1;
                    yearly_first_second_third_fourth_day_of_month = true;
                } else if (r.bysetpos[0] == -1) {
                    // to handle last day
                    yearly_lastday_of_month = true;
                }
            }
        }
        ...
        try {
            ...
            if (r.until != null) {
                ...
                // 15 is length of date-time without trailing Z e.g.
                // "20090204T075959"
                // A string such as 20090204 is a valid UNTIL (see RFC 2445) and
                // the
                ...
                // timezone as all the generated dates so that we can compare
                // them
                ...
                Time untilTime = new Time(dtstart);
                untilTime.year = 2036;
                untilTime.month = 11;
                untilTime.monthDay = 31;
                // change @siso kanchan P140626-02492 fix start
                try {
                    untilTime.normalize(true);
                } catch (IllegalArgumentException e) {
                    Log.i(TAG, "Calling unsafe normalize");
                    unsafeNormalize(untilTime);
                }
                // change @siso kanchan P140626-02492 fix end
                // code modified by Raghu. if untill is null then set the untill
                // to max date dec 31 2036
                untilDateValue = normDateTimeComparisonValue(untilTime);
                // -
            }
            ...
            if (EmailLog.DEBUG) {
                Log.i(TAG, "expand called w/ rangeStart=" + rangeStartDateValue + " rangeEnd="
                        + rangeEndDateValue);
            }
            ...
            events: {
                while (true) {
                    ...
                    if (EmailLog.DEBUG)
                        Log.i(TAG, " iteratorMonth:" + iteratorMonth + " iteratorDay:"
                                + iteratorDay + " iteratorYear:" + iteratorYear);
                    // year is never expanded -- there is no BYYEAR
                    generated.set(iterator);

                    if (EmailLog.DEBUG)
                        Log.i(TAG, "year=" + generated.year);
                    if (generated.year > 2036) {
                        break events;
                    }
                    do { // month
                        ...
                        // Use this to handle weeks that overlap the end of the
                        // month.
                        // Keep the year and month that days is for, and
                        // generate it
                        // when needed in the loop
                        if (useDays) {
                            // Determine where to start and end, don't worry if
                            // this happens
                            // to be before dtstart or after the end, because
                            // that will be
                            // filtered in the inner loop
                            if (freq == EventRecurrence.WEEKLY) {
                                /*
                                 * iterator.weekDay indicates the day of the
                                 * week (0-6, SU-SA). Because dayIndex might
                                 * start in the middle of a week, and we're
                                 * interested in treating a week as a unit, we
                                 * want to move backward to the start of the
                                 * week. (This could make the dayIndex negative,
                                 * which will be corrected by normalization
                                 * later on.) The day that starts the week is
                                 * determined by WKST, which defaults to MO.
                                 * Example: dayIndex is Tuesday the 8th, and
                                 * weeks start on Thursdays. Tuesday is day 2,
                                 * Thursday is day 4, so we want to move back (2
                                 * - 4 + 7) % 7 = 5 days to the previous
                                 * Thursday. If weeks started on Mondays, we
                                 * would only need to move back (2 - 1 + 7) % 7
                                 * = 1 day.
                                 */
                            ...
                            if (EmailLog.DEBUG)
                                Log.i(TAG, "dayIndex=" + dayIndex + " lastDayToExamine="
                                        + lastDayToExamine + " days=" + days);
                        }
                        if (EmailLog.DEBUG)
                            Log.i(TAG, "Monthly_Lastday:" + Monthly_Lastday + " Monthly_weekend:"
                                    + Monthly_weekend + " Monthly_week_day:" + Monthly_week_day
                                    + " Monthly_Lastday:" + Monthly_Lastday);

                        if (EmailLog.DEBUG)
                            Log.i(TAG, "yearly_anyday:" + yearly_anyday + " yearly_weekday:"
                                    + yearly_weekday + " yearly_weekend:" + yearly_weekend
                                    + " yearly_first_second_third_fourth_day_of_month:"
                                    + yearly_first_second_third_fourth_day_of_month
                                    + " yearly_lastday_of_month:" + yearly_lastday_of_month);

                        do { // day
                            int day;
                            if (useDays) {
                                if (Monthly_Lastday == true) {
                                    // get the last of month 31, 30, for feb 28,
                                    // 29
                                    day = days.getlastday(iterator);
                                    dayIndex = day;
                                } else if (Monthly_weekend == true) {
                                    // get the weekend day may be first or
                                    // second or third or fourth or last
                                    day = days.getweekend_day(iterator);
                                    dayIndex = days.getlastday(iterator);
                                } else if (Monthly_week_day == true) {
                                    // get the week day, may be first or second,
                                    // third, fourth or last
                                    day = days.getweek_day(iterator);
                                    dayIndex = days.getlastday(iterator);
                                } else {
                                    if (yearly_anyday == true
                                            || yearly_weekday == true
                                            || yearly_weekend == true
                                            || yearly_first_second_third_fourth_day_of_month == true
                                            || yearly_lastday_of_month == true) {
                                    ...
                                    if (yearly_weekday == true) {
                                        day = days.getweek_day(iterator);
                                        dayIndex = 31; // to avoid more loops
                                    } else if (yearly_weekend == true) {
                                        day = days.getweekend_day(iterator);
                                        dayIndex = 31; // to avoid more loops
                                    } else if (yearly_lastday_of_month == true) {
                                        day = days.getlastday(iterator);
                                        dayIndex = 31; // to avoid more loops
                                    } else {
                                        if (r.freq == EventRecurrence.YEARLY) {
                                            lastDayToExamine = iterator
                            ...
                            do {
                                ...
                                int minuteIndex = 0;
                                do {
                                    int minute = usebyminute ? r.byminute[minuteIndex]
                                            : iteratorMinute;
                                    if (EmailLog.DEBUG)
                                    ...
                                    int secondIndex = 0;
                                    do {
                                        int second = usebysecond ? r.bysecond[secondIndex]
                                                : iteratorSecond;
                                        if (EmailLog.DEBUG)
                                            Log.i(TAG, "          second=" + second);

                                        // we do this here each time, because if
                                        // we distribute it, we find the
                                        // month advancing extra times, as we
                                        // set the month to the 32nd, 33rd, etc.
                                        // days.
                                        generated.set(second, minute, hour, day, month,
                                                iteratorYear);
                                        unsafeNormalize(generated);

                                        long genDateValue = normDateTimeComparisonValue(generated);
                                        // sometimes events get generated
                                        // (BYDAY, BYHOUR, etc.) that
                                        // are before dtstart. Filter these. I
                                        // believe this is correct,
                                        // but Google Calendar doesn't seem to
                                        // always do this.
                                        if (genDateValue >= dtstartDateValue) {
                                            // filter and then add
                                            // TODO: we don't check for stop
                                            // conditions (like
                                            // passing the "end" date) unless
                                            // the filter
                                            // allows the event. Could stop
                                            // sooner.
                                            ...
                                            if (0 == filtered) {
                                                ...
                                                // (for RRULEs -- additive).
                                                // This condition must be the
                                                // complement of the
                                                // condition for incrementing
                                                // count at the
                                                // beginning of the method, so
                                                // if we don't
                                                // increment count there, we
                                                // increment it here.
                                                // For example, if add is set
                                                // and dtstartDateValue
                                                // is inside the start/end
                                                // range, then it was added
                                                // and count was incremented at
                                                // the beginning.
                                                // If dtstartDateValue is
                                                // outside the range or add
                                                // is not set, then we must
                                                ...
                                                if (genDateValue >= rangeStartDateValue) {
                                                    if (EmailLog.DEBUG) {
                                                        Log.i(TAG, "adding date=" + generated
                                                                + " filtered=" + filtered);
                                                    }
                                                    if (add) {
                    ...
                    // Add freqAmount to freqField until we get another date
                    // that we want.
                    // We don't want to "generate" dates with the iterator.
                    // XXX: We do this for days, because there is a varying
                    // number of days
                    // per month
                    int oldDay = iterator.monthDay;
                    generated.set(iterator); // just using generated as a
                                             // temporary.
                    int n = 1;
                    while (true) {
            ...
            // + sec
            Monthly_Lastday = false;
            Monthly_weekend = false;
            Monthly_week_day = false;
            yearly_anyday = false;
            yearly_weekday = false;
            yearly_weekend = false;
            yearly_lastday_of_month = false;
            yearly_first_second_third_fourth_day_of_month = false;
    ...
    /**
     * Normalizes the date fields to give a valid date, but if the time falls in
     * the invalid window during a transition out of Daylight Saving Time when
     * time jumps forward an hour, then the "normalized" value will be invalid.
     * <p>
    ...
    static void unsafeNormalize(Time date) {
        ...
        // At this point, monthDay >= 1. Normalize the month to the range
        // [0,11].
        if (month < 0) {
     ...
     * values to longs provides a total ordering on the Time values so that two
     * Time values can be compared efficiently by comparing their 64-bit long
     * values. This is faster than converting the Time values to UTC
     * millliseconds.
     * 
    ...
    private static final long normDateTimeComparisonValue(Time normalized) {
        ...
        return ((long)normalized.year << 26) + (normalized.month << 22)
                + (normalized.monthDay << 17) + (normalized.hour << 12) + (normalized.minute << 6)
                + normalized.second;
    }
