...
public class BufferedLineReaderInputStream extends LineReaderInputStream {
    ...
    public String toString() {
        ...
        buffers.append("[pos: ");
        buffers.append(this.bufpos);
        buffers.append("]");
        buffers.append("[limit: ");
        buffers.append(this.buflen);
        buffers.append("]");
        buffers.append("[");
        for (int i = this.bufpos; i < this.buflen; i++) {
            buffers.append((char) this.buffer[i]);
        }
        buffers.append("]");
        if (tempBuffer) {
            buffers.append("-ORIG[pos: ");
            buffers.append(this.origBufpos);
            buffers.append("]");
            buffers.append("[limit: ");
            buffers.append(this.origBuflen);
            buffers.append("]");
            buffers.append("[");
            for (int i = this.origBufpos; i < this.origBuflen; i++) {
                buffers.append((char) this.origBuffer[i]);
            }
            buffers.append("]");
        }
