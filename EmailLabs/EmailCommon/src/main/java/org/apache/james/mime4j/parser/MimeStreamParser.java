...
public class MimeStreamParser {
    ...
    private boolean collectPart = false;
        ...
        OUTER: for (;;) {
            ...
            switch (state) {
                ...
                case T_START_MULTIPART:
                    BodyDescriptor bd = mimeTokenStream.getBodyDescriptor();
                    handler.startMultipart(bd);

                    /* extract the detached content if the message is signed */
                    if(collectPart && (null != bd) && bd.getMimeType().contains("multipart/signed")) {
                    	String protocol = bd.getProtocol();
                    	if((null != protocol) && protocol.contains("application/pgp-signature")) {
                      		state = mimeTokenStream.next();
                      		if(state.equals(EntityState.T_PREAMBLE)) {
                      		  handler.preamble(mimeTokenStream.getInputStream());
                      		  state = mimeTokenStream.next();
                      		}
                    		switch (state) {
                    			case T_START_BODYPART:
                    				handler.body(mimeTokenStream.getInputStream());
                               		state = mimeTokenStream.next();
                                    break;
                    		}
                    	}
